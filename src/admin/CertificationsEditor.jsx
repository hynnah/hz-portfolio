...
export default function CertificationsEditor({ certifications, reload }) {
  ...
  const save = async (idx) => {
    ...
    setBusyId(null);
    showToast(result.ok ? 'Certification saved.' : result.message, result.ok);
    if (result.ok) reload();
  };
  ...
  return (
    <section>
      <div className="admin__panel-toolbar">
        <div>
          <h2 className="admin__panel-title">Certifications</h2>
          <p className="admin__panel-hint">Shown as cards under About.</p>
        </div>
            ...
            <div key={key} className="admin__card">
              <div className="admin__row">
                ...
                <label className="field">
                  <span>Issuer</span>
                  <input className="input" value={row.issuer} onChange={(e) => update(idx, { issuer: e.target.value })} />
                </label>
                <label className="field" style={{ flex: '0 1 100px' }}>
                  <span>Year</span>
                  <input className="input" value={row.year} onChange={(e) => update(idx, { year: e.target.value })} />
                </label>
                <label className="field" style={{ flex: '0 1 100px' }}>
                  <span>Order</span>
      ...
      <div className="admin__toolbar" style={{ justifyContent: 'flex-start', marginTop: 16 }}>
        <button type="button" className="btn btn-secondary admin__add-btn" onClick={addRow}>+ Add certification</button>
      </div>
    </section>
  );
}
